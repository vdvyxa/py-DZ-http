...
class YaUploader:
    ...
    #                (по умолчанию - 'token')
    def __init__(self, token = '', token_file = 'token'):
        self.token = token
...
if __name__ == '__main__':
    ...
    files_path = input("Enter relative path to files: ")
    files_count = get_files_count(files_path)
    destination_path = 'vdvyxa'
    token = ''
    
    uploader = YaUploader(token)
    result = uploader.upload_files(files_path, destination_path)
    print(f'Totat {result} files of {files_count} uploaded.')
